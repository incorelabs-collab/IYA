...
$(document).ready(function() {
    ...
    app.db.transaction(function (tx) {
        ...
        tx.executeSql("SELECT year, name, member_id FROM past_chairmen", [],
            function (tx, r) {
                for(var i =0;i< r.rows.length; i++) {
                    chairmenConcatString += "<br/><div class='row pastLeadersBlock'>";
                    if(r.rows.item(i).member_id == null) {
                        chairmenConcatString += "<div class='col-xs-4 pastLeadersImgBlock'><img src='img/customer.png' class='img-responsive'></div><div class='col-xs-8'><span class='noLinkMember'>";
                    } else {
                        chairmenConcatString += "<div class='col-xs-5 pastLeadersImgBlock'><img src='"+imgDir+r.rows.item(i).member_id+".jpg' class='img-responsive' onerror='pagePastLeaders.imgError(this)'></div><div class='col-xs-7'><a onclick='pagePastLeaders.getParentPage("+r.rows.item(i).member_id+")' class='memberLink'><span>";
                    }
                    chairmenConcatString += r.rows.item(i).name+"</span></a><br/><span class='tenureLink'>("+r.rows.item(i).year+")</span></div></div>";
                    $("#pastChairmenData").append(chairmenConcatString);
                    chairmenConcatString = "";
                }
                chairmenConcatString = "";
            }, app.dbQueryError
        );
